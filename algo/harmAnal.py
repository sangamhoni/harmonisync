        ...
        if key != prevKey:
            for m in range(prevKC, n + 3):
                ...
                if mKey == key:
                    ...
                    first_note_or_rest = (
                        cur.flat.notes.stream().first()
                        or cur.flat.rests.stream().first()
                    )
...
def moveChord(c: chord.Chord) -> chord.Chord:
    ...
    transposed_highest = highest_pitch.transpose(
        interval.Interval("-P8")
    )  # takes highest pitch down an octave
...
def min7(ch: chord.Chord) -> chord.Chord:
    ...
    for i in range(len(pitches)):
        for j in range(i + 1, len(pitches)):
            ...
            if inter.name == "M2" or inter.name == "m2":
                intervals[
                    f"{pitches[i].nameWithOctave} to {pitches[j].nameWithOctave}"
                ] = inter.name
                r = j
                y = 2
    ...
    if len(intervals) == 1:
        if y == 2:
            for k in range(
                len(pitches) - r
            ):  # modify the chord so that root notes is the higher note of the 2nd
                ch = moveChord(ch)
        if y == 7:
            for k in range(
                len(pitches) - r
            ):  # modify the chord so that root notes is the lower note of the 7th
                ch = moveChord(ch)
        ch = ch.transpose("P8")
...
def notesToRoman(set: pitch, curKey):
    ...
    if (
        roman.romanNumeralFromChord(ch, curKey).functionalityScore == 0
        or roman.romanNumeralFromChord(ch, curKey.parallel).functionalityScore == 0
    ):
        x = 1
        chh = moveChord(ch)
        while x < len(ch.pitches):
            if (
                roman.romanNumeralFromChord(chh, curKey).functionalityScore > 0
                or roman.romanNumeralFromChord(ch, curKey.parallel).functionalityScore
                > 0
            ):
...
def anaHarm(score, condensed, loc):
    ...
    for i in range(sum(1 for _ in score.parts[0].getElementsByClass("Measure"))):
        if any(
            isinstance(el, meter.TimeSignature) for el in score.measure(1).notesAndRests
        ):  # in case of meter changes
        ...
        for beatStart in range(int(dur)):
            ...
            beatNotes = inMeasure.getElementsByOffset(
                beatStart, beatStart + div, includeEndBoundary=False
            ...
            beatChord.quarterLength = (
                div  # Set duration to 1 quarter note (or other depending on meter)
            )

            # Analyze the Roman numeral for this beat
            beatRN = notesToRoman(
                combined_pitches, curKey
            )  # Analyzes harmony of pitches in each beat
...
def analyze(score):
    ...
    outPart = anaHarm(
        score, xx, loc
    )  # analyzes harmony of simplified input, produces output
    return outPart
...
def voicing(chorD):
    ...
    voiced.append(findPitch(note.Note("E3"), f, chordPitches))
    voiced.append(findPitch(note.Note("A3"), f, chordPitches))
    voiced.append(findPitch(note.Note("D4"), f, chordPitches))
    voiced.append(findPitch(note.Note("G4"), f, chordPitches))
    voiced.append(findPitch(note.Note("B4"), f, chordPitches))
    voiced.append(findPitch(note.Note("E5"), f, chordPitches))
    ...
    for i in range(sum(1 for _ in outPart.getElementsByClass("Measure"))):
        ...
        for measure in measures:
            for chord_item in measure.getElementsByClass(chord.Chord):
                ...
                quality = harmony.chordSymbolFigureFromChord(
                    chord_item
                )  # Get quality (e.g., "maj", "m7")
